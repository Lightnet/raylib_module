...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        float size = 0.1f;  // Adjustable collision spacing for walls
        Vector3 wallCheckPositions[4] = {
            {player.position.x, player.position.y, player.position.z + size},  // Forward
            {player.position.x, player.position.y, player.position.z - size},  // Back
            {player.position.x - size, player.position.y, player.position.z},  // Left
            {player.position.x + size, player.position.y, player.position.z}   // Right
        };
        ...
        float ssize = 0.01f;  // Tight fall check offset

        // Check 8 directions around player plus directly below for support
        Vector3 supportCheckPositions[9] = {
            {player.position.x, player.position.y - ssize, player.position.z},          // Below
            {player.position.x + ssize, player.position.y - ssize, player.position.z},  // Below-right
            {player.position.x - ssize, player.position.y - ssize, player.position.z},  // Below-left
            {player.position.x, player.position.y - ssize, player.position.z + ssize},  // Below-forward
            {player.position.x, player.position.y - ssize, player.position.z - ssize},  // Below-back
            {player.position.x + ssize, player.position.y - ssize, player.position.z + ssize}, // Below-right-forward
            {player.position.x + ssize, player.position.y - ssize, player.position.z - ssize}, // Below-right-back
            {player.position.x - ssize, player.position.y - ssize, player.position.z + ssize}, // Below-left-forward
            {player.position.x - ssize, player.position.y - ssize, player.position.z - ssize}  // Below-left-back
        };
        ...
        // Update ground block color
        for (int i = 0; i < blockCount; i++) {
            if (i == groundIndex) {
                blocks[i].color = ORANGE;
            } else if (blocks[i].color.r == 255 && blocks[i].color.g == 200) {
                blocks[i].color = GRAY;
            }
        }
